...
| `db.driver` | `auto` | `auto` \| `jdbc` \| `sqlcmd` - see below |
...
| `split.renameFrom` / `split.renameTo` | | Redirect the script at a differently-named database - see below |
| `run.commitEvery` | `1000` | Statements per transaction/commit (JDBC mode only) |
...
This per-statement resume is how the **JDBC** driver (`db.driver=jdbc`) works.
The **`sqlcmd`** driver resumes per *part* instead: each pure-DML part runs
inside one transaction (`SET XACT_ABORT ON` + `BEGIN/COMMIT TRANSACTION`), so
a failure rolls back the whole part and a retry is always safe - it just
doesn't know the exact statement that failed, only the part. `db.driver=auto`
(the default) picks `sqlcmd` whenever it resolves on `PATH`, because it can
reach a local SQL Server over Named Pipes/Shared Memory where the JDBC driver
cannot (see [Troubleshooting](#troubleshooting)).

## Redirecting a script at a different database

A script generated by SSMS or GeneXus has its target database name baked
into `CREATE DATABASE`/`USE`/`ALTER DATABASE` statements - and into the
physical `.mdf`/`.ldf` paths inside `CREATE DATABASE` - as literal text.
There is no server-side setting that redirects that; to test-import a script
into a scratch database without touching the one it was generated from, the
text has to change. Set:

```properties
split.renameFrom=GX_KB_PARQUE-SALUD-Ev3
split.renameTo=GX_KB_PARQUE-SALUD-Ev3_TEST
```

and `split` rewrites every occurrence (literal substring match, not a regex)
before writing it to a part - which also fixes the physical file paths for
free, since they contain the same substring. Leave `split.renameFrom` blank
(the default) to leave the script untouched.
...
This means it's using the JDBC driver (`db.driver=jdbc`, or `auto` with
`sqlcmd` not found on `PATH`) against a server that only has Named
Pipes/Shared Memory enabled - common on a fresh local SQL Server Express
install. Easiest fix: install the `sqlcmd` command-line tool (part of the
[SQL Server command-line utilities](https://learn.microsoft.com/sql/tools/sqlcmd/sqlcmd-utility))
and leave `db.driver=auto` (the default) - NanoSplit will use it instead,
and it connects locally without TCP/IP. To use the JDBC driver anyway,
enable TCP/IP: **SQL Server Configuration Manager** → SQL Server Network
Configuration → Protocols for `<your instance>` → enable **TCP/IP** →
restart the SQL Server service (also start **SQL Server Browser** for a
named instance, or set a fixed port under TCP/IP → IPAll).

**A part fails with "Windows NT user or group '...' not found"**

The script has a `CREATE USER [DOMAIN\Group]` or `ALTER ROLE ... ADD MEMBER
[DOMAIN\Group]` statement mapped to a Windows domain group from the machine
the script was generated on. If you're restoring onto a different machine
(not joined to that domain, or a workgroup PC), SQL Server cannot resolve
the group and that one statement fails - it has nothing to do with your data.
Comment out (`--`) those specific `CREATE USER`/`ALTER ROLE ADD MEMBER`
lines in the affected part and re-run; everything else is unaffected.
